...
ETUDE = [
    ...
    {'s':'Is2',       'l':('33',0), 'r':('24',0)},           # Is2 + I root (add9)
    {'s':'Is4',       'l':('42',0), 'r':('424',0)},         # Is4 paired
    ...
    {'s':'IVs2',      'l':('33',3), 'r':('24',3)},          # IVs2 + IV root (add9)
    ...
    {'s':'Vq',        'l':('33',4), 'r':('44',4)},          # Vq + V root triad
    {'s':'IVΔ',       'l':('323',0),'r':('434',2)},         # IVΔ (2nd inv LH + 3rd inv alt RH)
    # ── Bar 13: Sus2 phrase ──
    {'s':'iis2',      'l':('33',1), 'r':('24',1)},          # + ii root
    {'s':'Vs2',       'l':('33',4), 'r':('24',4)},          # + V root
    {'s':'vis2',      'l':('33',5), 'r':('24',5)},          # + vi root
    # ── Bar 14: Sus4 chain + V7 ──
    {'s':'iis4',      'l':('42',1), 'r':('424',1)},
    {'s':'Vs4',       'l':('42',4), 'r':('424',4)},
    {'s':'V7',        'l':('323',1),'r':('434',3)},         # V7 (2nd inv LH + 3rd inv alt RH)
    # ── Bar 15: Color solos ──
    {'s':'iiis4',     'l':('33',2), 'r':('42',2)},          # + iii root
    {'s':'vis4',      'l':('33',5), 'r':('42',5)},          # + vi root
...
def generate_html(abc_text, key_root=7):
    return _shared_generate_html(abc_text, key_root, ETUDE, title='Etude (v2)')
...
if __name__ == '__main__':
    ...
    abc_path = os.path.join(os.path.dirname(__file__), '..', 'handout', 'etude2.abc')
    with open(abc_path, 'w') as f:
        f.write(abc)
    print(f'Written {abc_path}')

    html_path = os.path.join(os.path.dirname(__file__), '..', 'handout', 'etude2.html')
